...
### ~4 fonds multi-échelle (paliers ×100/×20) — re-fetch
- **Priorité** : ⚪ Mineure
- **Détecté le** : 2026-06-25 (trié dans la foulée du recalage yahoo→justetf)
- **Où** : `investissement_fund_prices` — First Trust Global Equity Income `IE00BD842Y21`, First Trust NextG `IE00BWTNM743`, First Trust DJ Internet `IE00BT9PVG14`, BlueSphere World Equity CZK Hedged `IE000YC7FPB6`
- **Le problème** : ces séries ont un **palier d'échelle** (×100 ou ×20) au milieu (ex. FTGEI ~32 → ~3200 et y reste ; BlueSphere ~185 → ~9,1 dans justetf), pas un pic isolé → le détecteur de pics ne les vise pas (à raison : ils ne reviennent pas). Fixer la **bonne** échelle sans deviner demande une référence externe.
- **Comment l'aborder** : re-fetch ciblé de la VL réelle (geco/justetf/issuer) → rescale du segment fautif, comme la catégorie « multi-échelle/régime » du 23/06. 4 fonds, faible enjeu.
- **Effort estimé** : rapide
- **Déjà traité (✅ Réglés)** : sur les 26 fonds résiduels triés, **18 pics single-point réparés** (interpolation) + **~4 crypto/bitcoin conservés** (vrais mouvements) ; restent ces 4 multi-échelle.
