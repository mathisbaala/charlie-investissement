...
export default async function InsurerPage({
  ...
  const [comparison, profileRes, feRes] = await Promise.all([
    ...
    supabase
      ...
      .select(
        "kind, groupe, positionnement, fonds_euros, forces, limites, solvabilite_2_pct, notation, notation_agence, notation_annee, ppb_pct, ppb_annee, encours_vie_mds, encours_annee, sfcr_annee, sfcr_url",
      )
      .eq("company", company)
