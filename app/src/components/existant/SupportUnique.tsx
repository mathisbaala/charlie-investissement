...
// ─── Chemin « support unique » de l'analyse ─────────────────────────────────────
// Déposer un DICI / KID → rapport de fonds structuré (ex-onglet Documents).
// Rendu sans PageShell : encapsulé par AnalyseExistant, qui porte l'en-tête et le
// sélecteur de mode (portefeuille ↔ support unique).
export function SupportUnique() {
  ...
  return (
    <>
      ...
      {/* Drop zone — masquée une fois le rapport affiché */}
      {!fiche && !loading && (
        <div
          onDrop={handleDrop}
        ...
        >
          ...
          <div>
            <p className="text-body-lg font-medium text-ink-2">
              {dragging ? "Relâchez pour analyser" : "Glissez le DICI d'un support ici"}
            </p>
