...
**Correction (vérifié 2026-07-10)** : **La Banque Postale Life n'est PAS un trou** —
elle est couverte par `av-lux-opcvm360-catalog.py` (KNOWN_CONTRACTS 680 + 681,
clé iframe Meilleurtaux/MonFinancier). Dry-run OK : *Compte Libre Croissance LBP*
**et** *LBP 2* rendent 49 fonds chacun. Le 2ᵉ (681) est codé mais pas encore en base
→ même cas que ACM/MACSF/Afer (attend un run CI). La clé opcvm360 n'expose que ces
2 contrats LBP ; aller au-delà de 49 UC exigerait une **autre clé distributeur** ou
une annexe CNP Luxembourg — non trouvée en source publique/gratuite.

**Cas marginaux portail-only** (non ajoutables en statique gratuit) : Macif Multi Vie
(portail JS anti-bot), contrats Utmost hors « Liberté » (espace courtier).
...
| 13 | La Banque Postale Life | `av-lux-opcvm360-catalog.py` (680+681) | API opcvm360 | 49 fonds × 2 contrats | ✅ couvert (LBP 2 pending apply) |
...
| La Banque Postale Life | 49 | 1 | ✅ opcvm360 (680+681) ; manque « LBP 2 » (codé) |
| Carac | 29 | 2 | ✅ |
...
1. **La Banque Postale Life — déjà couverte (pas un manque de scraper).** Vérifié :
   `av-lux-opcvm360-catalog.py` la scrape via 2 contrats (680 « Compte Libre Croissance
   LBP », 681 « … LBP 2 »), 49 fonds chacun en dry-run. Le contrat 681 est codé mais
   pas encore en base (attend un `--apply`). **Limite réelle** : la couverture plafonne
   à ~49 UC parce que la clé opcvm360 (distributeur Meilleurtaux/MonFinancier) n'expose
   que ces 2 contrats. Élargir exigerait une **autre clé iframe** (un distributeur
   différent) ou une **annexe CNP Luxembourg** — aucune source **publique et gratuite**
   trouvée (les contrats grand public LBP — Cachemire, Vivaccio, Solésio — sont assurés
   par **CNP** et déjà couverts par `av-fr-cnp-catalog.py`). **Reco** : ne rien coder de
   nouveau ; si Mathis a une clé iframe d'un autre distributeur LBP, l'ajouter à
   `KNOWN_CONTRACTS`.
