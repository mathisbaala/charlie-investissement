...
| Orchestrateurs | `scripts/cron/weekly-pipeline.py` + `monthly-pipeline.py` (cadence réelle via GitHub Actions, cf. §6), `scripts/cron/overnight-mass-scrape.py` (seeding manuel) |
...
# Phase 2 : enrichissement Morningstar (holdings/perfs)
# Désormais automatisé : workflow holdings-drain.yml (à la demande, shardé) et
# étape Morningstar du monthly-pipeline.py. Les anciens scripts shell de poll
# manuel (wait-and-run/post-morningstar/watch-*) ont été retirés.
...
| Script | Rôle | Usage |
|--------|------|-------|
| `cron/weekly-pipeline.py` | Rafraîchissement hebdo (prix FT/JustETF/GECO + crypto + métriques + alpha). Cadence réelle via `weekly-refresh.yml` | `python3 scripts/cron/weekly-pipeline.py` |
| `cron/monthly-pipeline.py` | Enrichissement mensuel fill-only (holdings, breakdowns, SFDR, TD). Via `monthly-enrich.yml` | `python3 scripts/cron/monthly-pipeline.py` |
| `cron/overnight-mass-scrape.py` | Orchestrateur de SEEDING (phase 1→4), **destructif** — manuel uniquement, jamais en CI | `python3 scripts/cron/overnight-mass-scrape.py [--phase N]` |

> **Retiré (juin 2026)** : les scripts shell de poll/watcher manuels de l'ère du
> scraping nocturne (`wait-and-run-morningstar.sh`, `post-morningstar-pipeline.sh`,
> `watch-*.sh`, `monitor-and-trigger.sh`, `autopilot-nuit.sh`, `*-chain-*.sh`)
> liés à des PID figés et au chemin Yahoo déprécié. Remplacés par les workflows
> GitHub Actions (`weekly-refresh`, `monthly-enrich`, `holdings-drain`).
...
| Cron « 1er lundi » via `0 5 1-7 * 1` seul | GitHub Actions combine jour-du-mois ET jour-de-semaine en **OU** → se déclenche tous les lundis (throttle Morningstar, compute gaspillé) | Garder une garde dans le step (`date +%u`/`+%-d`), cf. `monthly-enrich.yml`/`emea-refresh.yml`/`classify-refresh.yml` |
| Réintroduire les VL Yahoo (`fetch-nav-yahoo`/`fetch-opcvm-nav`/`fetch-etf-prices`) en cadence régulière | VL périmées en concurrence des sources fraîches (FT/JustETF/GECO) — raison du retrait de `daily-pipeline.py` | Sources de prix = FT (top AUM + rotation), JustETF (filet ETF), GECO (univers FR) |
| Purger la « traîne » de fonds à prix périmé en les supprimant | La plupart sont des fonds vivants à gros encours (lacune de couverture), pas du garbage | Garde de péremption dans `compute-metrics.py` (purge perf/vol/Sharpe si dernier prix > 90 j **ET** `n_points>0`) — n'efface pas les fonds |
| Indice TR Yahoo via un ticker non couvert (`^XNDX`) | Yahoo renvoie ~1 point pour certains symboles TR → série inutilisable | Vérifier le nb de points ; préférer un ETF distribuant + `auto_adjust` (ex. `nasdaq100` = `QQQ`) |
...
# Drainer la composition Morningstar (à la demande, shardé)
#   onglet Actions → « Drain composition » → enricher=morningstar
