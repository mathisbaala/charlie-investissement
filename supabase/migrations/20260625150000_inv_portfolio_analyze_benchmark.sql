-- Moteur portefeuille — back-test (étape D) ──────────────────────────────────────
-- Étend inv_portfolio_analyze avec un BENCHMARK optionnel : la courbe d'un indice
-- de référence (investissement_index_prices) rejouée sur la MÊME grille hebdo que
-- le portefeuille, base 100, avec ses ratios. Permet la comparaison « portefeuille
-- vs marché » et la sur/sous-performance. La projection en euros se fait côté UI
-- (la courbe base 100 suffit).
--
-- Nouvelle signature (5 args) : p_benchmark = index_code de
-- investissement_index_catalog (ex. 'msci_world'). NULL → pas de benchmark.
-- On DROP l'ancienne signature 4-arg pour éviter une surcharge ambiguë côté PostgREST.

DROP FUNCTION IF EXISTS public.inv_portfolio_analyze(text[], numeric[], int, numeric);
...
BEGIN
  ...
  -- ─── Benchmark (optionnel) : même grille, LOCF depuis les indices ───
  bgrid AS (
    SELECT g.d,
...
COMMENT ON FUNCTION public.inv_portfolio_analyze(text[], numeric[], int, numeric, text) IS
  'Analyse un portefeuille pondéré : courbe composite hebdo (base 100), ratios, '
  'matrice de corrélation, et back-test optionnel vs un benchmark (p_benchmark = '
  'index_code). Multi-rythme (LOCF), mélange rééquilibré. Fonds sans VL exclus + signalés.';

REVOKE ALL ON FUNCTION public.inv_portfolio_analyze(text[], numeric[], int, numeric, text) FROM public;
GRANT EXECUTE ON FUNCTION public.inv_portfolio_analyze(text[], numeric[], int, numeric, text) TO anon, authenticated, service_role;
