-- Exclusions sectorielles ESG par fonds : la vraie donnée (EET) en base ────────
-- Contexte : le générateur d'allocation applique les exclusions client (tabac,
-- armes, fossiles, jeux, alcool) via un PROXY « labels » (ISR/Greenfin) faute de
-- donnée d'exclusion fonds par fonds. Ce chantier ajoute la donnée réelle,
-- collectée depuis les European ESG Templates (EET, FinDatEx) publiés par les
-- SGP — champs d'exclusions PAB/CTB (tabac, armes controversées, charbon/
-- fossiles) et politiques d'exclusion volontaires (jeux, alcool…).
--
--   esg_exclusions            jsonb  ex. {"tobacco":true,"controversial_weapons":true,
--                                         "thermal_coal":false,"gambling":true}
--                                    Clé présente = politique DOCUMENTÉE : true = le
--                                    fonds exclut le secteur, false = il ne l'exclut
--                                    pas. Clé absente = inconnu (→ le moteur replie
--                                    sur le proxy labels, cf. profileToConstraints.ts).
--   esg_exclusions_source     text   provenance (ex. « eet:amundi:2026-06 »)
--   esg_exclusions_updated_at date   date de référence de la donnée source (période EET)
--
-- Alimentation : scripts/scrapers/esg-exclusions-enricher.py (fill-only,
-- update_funds_bulk — ne crée jamais de fonds).

ALTER TABLE investissement_funds
  ADD COLUMN IF NOT EXISTS esg_exclusions            jsonb,
  ADD COLUMN IF NOT EXISTS esg_exclusions_source     text,
  ADD COLUMN IF NOT EXISTS esg_exclusions_updated_at date;

COMMENT ON COLUMN investissement_funds.esg_exclusions IS
  'Exclusions sectorielles documentées (EET/SFDR) : {clé: bool}. Clés canoniques : tobacco, controversial_weapons, weapons, thermal_coal, fossil, nuclear, gambling, alcohol, adult_entertainment, ungc_violations. Clé absente = inconnu.';

-- ── Vue légère (screener) : colonne APPENDUE en fin ──────────────────────────
...
-- + esg_exclusions en dernière position.
...
-- 20260706120000, + esg_exclusions jsonb avant relevance. On reproduit le
